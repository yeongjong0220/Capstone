...
// --- 기존 로직 (유지) ---
const BACK_FINDPW = import.meta.env.VITE_BACK_FINDPW;
...
const FindPw = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.patch(BACK_FINDPW, findPwData)
      .then((res) => {
        console.log(res);
        alert('비밀번호 변경 메일을 보냈습니다'); // (TODO: 나중에 커스텀 모달로 바꾸는 것을 권장)
        nav('/');
      })
      .catch((err) => {
        console.log(err);
        alert("비밀번호 변경 메일 전송에 실패했습니다"); // (TODO: 나중에 커스텀 모달로 바꾸는 것을 권장)
      });
  ...
  return (
    <AuthContainer>
      <AuthForm as="form" onSubmit={handleSubmit}> {/* <form> 태그로 작동하도록 'as' prop 사용 */}
        ...
        {/* --- 비밀번호 찾기 버튼 --- */}
        <StyledButton type="submit">찾기</StyledButton>
