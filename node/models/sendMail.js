...
const { NODEMAILER_USER, NODEMAILER_PASS, BACK_SERVER, FRONT_SERVER } = process.env;
...
const sendPwMail = async (email,code) => {
  ...
  const verificationUrl = `${FRONT_SERVER}/reset-pw?code=${code}`; // 이걸 비밀번호 변경할 수 있도록 바꿔야할 듯, 프론트 입력창으로
  const mailOptions = {
    from: NODEMAILER_USER,
    to: email,
    subject: "Code 비밀번호 변경", // 메일제목
    html: `<h1>Code 비밀번호 변경하기:</h1>
